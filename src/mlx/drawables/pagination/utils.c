...
static t_point2d		pagination_center(t_pagination *pagination, t_point2d pos, t_dim2d griddim)
{
	...
	overflow.width = pagination->zone.dim.width
		- (griddim.width * (pagination->items_dim.width + pagination->items_margin[3] / 2+ pagination->items_margin[1]));
...
t_dim2d					mlx_pagination_gride_dims(t_pagination *pagination)
{
	...
	dim.height = (pagination->zone.dim.height - MLX_PAGINATION_BOT_SIZE ) / (pagination->items_dim.height + pagination->items_margin[0] + pagination->items_margin[2]);
	return (dim);
}
